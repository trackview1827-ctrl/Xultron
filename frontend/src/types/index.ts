...
export interface User { id: string; username: string; email: string; isGuest: boolean; createdAt: string }
export interface SessionResponse { user: User | null; csrfToken: string; expiresAt: string | null }
export interface SessionDevice { id: string; current?: boolean; createdAt: string; lastSeenAt?: string; userAgent?: string; ipHint?: string }
export interface Conversation { id: string; title: string; createdAt: string; updatedAt: string }
export interface Message { id: string; conversationId: string; role: 'user' | 'assistant' | 'system'; content: string; createdAt: string; pending?: boolean; failed?: boolean }
export interface ProviderCredential { configured: boolean; masked: string }
export interface Provider {
  id: string; name: string; kind: ProviderKind; adapter: string; baseUrl: string; model: string;
  temperature?: number; maxTokens?: number; streaming?: boolean; enabled: boolean; isDefault: boolean;
...
export interface Device { id: string; name: string; type: string; status: string; lastSeenAt?: string }
