...
export function AuthGateway() {
  const { setUser, sessionReachable, settings, retryConnection } = useApp(); const [mode, setMode] = useState<'entry' | 'login' | 'register'>('entry')
  ...
  return <main className="auth-screen">
    <motion.section className="auth-core" initial={conserveMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: conserveMotion ? 0 : .24 }}><div className="brand-lockup"><span className="eyebrow">PERSONAL INTELLIGENCE SYSTEM</span><h1>XULTRON</h1></div><XultronCore state={sessionReachable ? 'ONLINE' : 'OFFLINE'} reducedMotion={conserveMotion} /></motion.section>
    <motion.section className="auth-panel" initial={conserveMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: conserveMotion ? 0 : .15, duration: conserveMotion ? 0 : .2 }}>
      ...
      {!sessionReachable && <div className="offline-note">Backend link unavailable. The cached interface is ready, but account access requires a connection.<button className="text-button" onClick={retryConnection}>RETRY BACKEND LINK</button></div>}
    </motion.section>
  </main>
}
