...
export function HomePage() {
  ...
  const send = async () => {
    const text = input.trim(); if (!text || streaming) return; if (!online) { setError(networkOnline ? 'The Xultron backend is unavailable. Retry the link before sending.' : 'Xultron is offline. Reconnect before sending.'); if (!networkOnline) dispatchCore({ type: 'NETWORK_LOST' }); return }
  ...
  return <div className="home-page">
    ...
    <section className="command-space">
      ...
      <motion.div layout={!conserveMotion} transition={{ duration: conserveMotion ? 0 : .3 }} className={`core-stage ${coreCompact ? 'compact' : ''}`}><XultronCore state={coreState} reducedMotion={conserveMotion} compact={coreCompact} level={voice.level} />{!coreCompact && <motion.div className="core-intro" initial={conserveMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: conserveMotion ? 0 : .2 }}><h1>How can I assist you?</h1><p>Voice, thought, and memory aligned.</p></motion.div>}</motion.div>
