...
export function AppShell() {
  const { page, setPage, online, user, settings } = useApp(); const usage = useDataUsage()
  return <div className="app-shell"><header className="topbar"><button className="wordmark" onClick={() => setPage('home')} aria-label="Go to Xultron Core"><span>X</span><strong>XULTRON</strong></button><div className="system-meta"><span className={online ? 'online' : 'offline'}><i />{online ? 'LINKED' : 'OFFLINE'}</span>{settings.lowDataMode && <span>LOW DATA</span>}<span className="desktop-only">↓ {formatBytes(usage.downloaded)} · ↑ {formatBytes(usage.uploaded)}</span><span className="identity">{user?.isGuest ? 'GUEST' : user?.username}</span></div></header>
    ...
    <main className="app-content"><AnimatePresence mode="wait" initial={false}><motion.div key={page} className="page-motion" initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -6 }} transition={{ duration: settings.reducedMotion ? 0 : .18 }}>{page === 'home' ? <HomePage /> : page === 'memory' ? <MemoryPage /> : <SettingsPage />}</motion.div></AnimatePresence></main>
    {!online && <div className="reconnect-strip" role="status"><Icon name="signal" /><span>NETWORK LINK LOST · WAITING TO RECONNECT</span></div>}
  </div>
}
