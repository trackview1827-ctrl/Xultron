...
class Message(TimestampMixin, db.Model):
    ...
    def to_public(self):
        ...
        attachments = self.meta.get("attachments") if isinstance(self.meta, dict) else None
        if isinstance(attachments, list):
            result["attachments"] = [_safe_attachment_metadata(item) for item in attachments if isinstance(item, dict)]
        return result


def _safe_attachment_metadata(value: dict) -> dict:
    """Expose only display metadata previously generated by the server."""
    allowed = ("id", "name", "contentType", "kind", "state", "size", "sha256", "extraction")
    return {key: value[key] for key in allowed if key in value}


class Attachment(TimestampMixin, db.Model):
    __tablename__ = "attachments"
    id = db.Column(db.String(40), primary_key=True, default=lambda: new_id("att"))
    user_id = db.Column(db.String(40), db.ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    name = db.Column(db.String(255), nullable=False)
    content_type = db.Column(db.String(100), nullable=False)
    kind = db.Column(db.String(16), nullable=False)
    state = db.Column(db.String(20), nullable=False, default="ready", index=True)
    size = db.Column(db.Integer, nullable=False)
    sha256 = db.Column(db.String(64), nullable=False)
    content = db.Column(db.LargeBinary, nullable=False)
    extracted_text = db.Column(db.Text, nullable=True)
    analysis_content = db.Column(db.LargeBinary, nullable=True)
    analysis_content_type = db.Column(db.String(100), nullable=True)
