...
from app.services.chat import create_conversation, handle_message, owned_conversation, persist_message, prepare_message, request_lock, stream_prepared_message
...
def stream_message():
    ...
    def events():
        try:
            ...
            stream_request_id = string_field(data, "requestId", required=True, min_len=1, max_len=100)
            with request_lock(user_id, stream_request_id):
...
@api_bp.get("/tasks/<task_id>/events")
def task_events_route(task_id):
    task = owned_task(task_id, require_user().id)
